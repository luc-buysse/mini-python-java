...
# partie 3i : tests d'ex̩cution avec interpr̬te
...
for f in exec/*.py; do
	...
	echo
	echo "ECHEC de l'interpr̩tation sur $f (devrait r̩ussir)"
    fi
done
echo
...
for f in exec-fail/*.py; do
	...
	echo
	echo "ECHEC : l'interpr̩tation sur $f devrait ̩chouer"
    else
	score_test=`expr $score_test + 1`;
	score_out=`expr $score_out + 1`;
    fi
